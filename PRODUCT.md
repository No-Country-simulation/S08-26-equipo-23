...
- Current dataset is synthetic but realistic — generated with Argentine seasonality patterns (winter flu peaks, spring allergies, summer-vacation lows) and real CABA neighborhood/socioeconomic profiles (~69,100 records over roughly 2 years), not measured historical data from an actual clinic.
...
- `data/healthdemand_buenos_aires.xlsx` — ~69,100 synthetic-but-realistic neighborhood-level records over ~2 years (Argentine seasonality + real CABA socioeconomic profiles, generated by the data team, not measured clinic data), fields (translated to English on load): `date, dayOfWeek, dayNumber, month, year, isHoliday, neighborhood, specialty, availableSlots, assignedTurns, attendedTurns, unmetDemand, cancelledTurns, noShowTurns, noShowRate, occupancyRate, saturationLevel, seasonFactor`. `saturationLevel` is an informational field carried by the dataset itself — it does not feed this app's own z-score anomaly calculation, kept as a separate signal. The earlier, less realistic synthetic dataset (`data/historical_turns.json` / `.csv`, arbitrary random generation with no real-world grounding) is retired; `generate-medical-data.js` and `load-to-postgresql.js` remain in the repo for reference but are no longer the load path in use.
